...
try:
    ...
    # For older torch versions where add_safe_globals does not exist
    pass
import copy
import warnings
...
def run_precision_comparison(device="cpu"):
    ...
    for dtype in precisions:
        ...
        x0.retain_grad()
        e_feats, _ = m.radial_embedding(
            lengths_d, batch_d.node_attrs, batch_d.edge_index, z_table
        )
        ...
        # Forward block 0
        print(
            "skip_tp.weight.dtype:",
            block0.skip_tp.weight.dtype,
            "node_feats.dtype:",
            inputs0["node_feats"].dtype,
        )
        ...
        if device.startswith("cuda"):
            ...
            m0 = 0
        output0_prod = product0(
            node_feats=output0, sc=None, node_attrs=inputs0["node_attrs"]
        )
        loss0 = (output0**2).sum()
        loss0.backward()
        ...
        if grad0 is None:
            raise RuntimeError(
                "Gradient for inputs0['node_feats'] is None. Ensure .retain_grad() is called on the tensor."
            )
        ...
        if grad1 is None:
            raise RuntimeError(
                "Gradient for inputs1['node_feats'] is None. Ensure .retain_grad() is called on the tensor."
            )
        results[1][dtype] = {
    ...
    df_perf.to_excel(
        f"Experiments/precision_3/xlsx/precision_metrics.xlsx", index=False
    )

    ns_plots.plot_precision_comparison(
        df_perf, "Experiments/precision_3/figs/precision_comparison.png"
    )
...
def test_tf32_vs_fp32(device="cuda"):
    ...
    # Test configurations
    configs = [("tf32", True, True), ("fp32_no_tf32", False, False)]
    ...
    for config_name, allow_matmul_tf32, allow_cudnn_tf32 in configs:
        ...
        for block_idx in [0, 1]:
            ...
            if block_idx == 0:
                ...
                output_prod = product(
                    node_feats=output, sc=None, node_attrs=inputs["node_attrs"]
                )
                node_feats_next = (
                    output_prod.clone()
                    .detach()
    ...
    df_perf = pd.DataFrame(metrics)
    df_perf.to_excel(
        "Experiments/precision_3/xlsx/tf32_vs_fp32_metrics.xlsx", index=False
    )

    # Plot the comparison
    ns_plots.plot_precision_comparison(
        df_perf, "Experiments/precision_3/figs/tf32_vs_fp32_comparison.png"
    )
    ...
    for block_idx in [0, 1]:
        ...
        # Forward error
        err_fwd = np.abs(
            tf32_results[block_idx]["output"] - fp32_results[block_idx]["output"]
        )
        rel_fwd = np.abs(err_fwd / (np.abs(tf32_results[block_idx]["output"]) + 1e-12))

        # Backward error
        err_bwd = np.abs(
            tf32_results[block_idx]["grad"] - fp32_results[block_idx]["grad"]
        )
        rel_bwd = np.abs(err_bwd / (np.abs(tf32_results[block_idx]["grad"]) + 1e-12))

        # Performance
        time_speedup = (
            fp32_results[block_idx]["time_ms"] / tf32_results[block_idx]["time_ms"]
        )
        memory_ratio = (
            tf32_results[block_idx]["memory_bytes"]
            / fp32_results[block_idx]["memory_bytes"]
        )
...
def comprehensive_precision_test(device="cpu"):
    ...
    for config_name, dtype, allow_matmul_tf32, allow_cudnn_tf32 in precision_configs:
        ...
        if device.startswith("cuda"):
            ...
            if allow_matmul_tf32 or allow_cudnn_tf32:
                print(
                    f"  TF32 enabled: matmul={allow_matmul_tf32}, cudnn={allow_cudnn_tf32}"
                )
        ...
        for block_idx in [0, 1]:
            block = m.interactions[block_idx]
            print(" cueq:", block.cueq_config)
            print(
                " oeq:",
                block.oeq_config,
                "conv_fusion:",
                getattr(block.oeq_config, "conv_fusion", None),
            )
            print(" conv_tp type:", type(block.conv_tp))
            print(" has conv_fusion attr?", hasattr(block, "conv_fusion"))
    ...
    for block_idx in [0, 1]:
        ...
        for config_name in results[block_idx].keys():
            ...
            comparison_data.append(
                {
                    ...
                    "memory_mb": results[block_idx][config_name]["memory_bytes"]
                    / 1024**2,
                }
            )
            ...
            print(f"  {config_name}:")
            print(
                f"    Forward error: {err_fwd.max():.2e} (abs), {rel_fwd.max():.2e} (rel)"
            )
            print(
                f"    Backward error: {err_bwd.max():.2e} (abs), {rel_bwd.max():.2e} (rel)"
            )
            print(f"    Speedup vs FP32: {speedup:.2f}x")
            print(f"    Time: {current_time:.2f}ms")
            print(
                f"    Memory: {results[block_idx][config_name]['memory_bytes']/1024**2:.1f}MB"
            )

    # Save comprehensive results
    df_comprehensive = pd.DataFrame(comparison_data)
    df_comprehensive.to_excel(
        "Experiments/numerical_stability/xlsx/comprehensive_precision_test.xlsx",
        index=False,
    )
    print(
    ...
    df_perf = pd.DataFrame(metrics)
    df_perf.to_excel(
        "Experiments/numerical_stability/xlsx/comprehensive_performance_metrics.xlsx",
        index=False,
    )
    ...
    ns_plots.plot_precision_comparison(
        df_perf,
        "Experiments/numerical_stability/figs/comprehensive_performance_comparison.png",
    )
    # Call error distribution plotting
    ns_plots.plot_error_distributions_2(
        results,
        batch,
        z_table,
        ref_precision="fp64",
        save_path="Experiments/numerical_stability/figs/comprehensive_error_distributions.png",
    )
