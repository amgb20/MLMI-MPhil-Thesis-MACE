"""
Benchmark for Linear Product (product.linear) in MACE

This benchmarks the linear layer within the product block across:
- Backends: e3nn (reference) and cuEq (if available)
- Precisions: fp64, fp32, bf16/fp16 where supported
- Batch sizes
- Forward and backward latency

Inputs to product.linear are constructed exactly as in models.product:
- For interaction block 0: we run interactions[0](...) -> products[0](...) and capture the output of symmetric_contractions
- For interaction block 1: we run the full pipeline through products[1](...) and capture the output of symmetric_contractions
...
def capture_linear_call_args(linear_block, run_callable):
    """Monkeypatch linear_block.forward to capture exact args passed during a normal forward."""
    captured = {}
    orig_forward = linear_block.forward
    def wrapper(*args, **kwargs):
        captured['args'] = args
        captured['kwargs'] = kwargs
        return orig_forward(*args, **kwargs)
    linear_block.forward = wrapper
    try:
        run_callable()
    finally:
        linear_block.forward = orig_forward
    return captured.get('args', ()), captured.get('kwargs', {})


def build_captured_args_for_linear_product(model, inputs, interaction_idx):
    # Prepare a run that executes the product[idx] as in the model to capture exact linear inputs
    prod_idx = interaction_idx
    linear_block = model.products[prod_idx].linear
    ...
    if interaction_idx == 0:
        def run():
            ...
            # Run the full product block to trigger linear with symmetric_contractions output
            _ = model.products[0](node_feats=out0, sc=sc0, node_attrs=inputs["node_attrs"])
        args, kwargs = capture_linear_call_args(linear_block, run)
    else:
        def run():
            out0, sc0 = model.interactions[0](
                node_attrs=inputs["node_attrs"],
            ...
            # Run the full product block to trigger linear with symmetric_contractions output
            _ = model.products[1](node_feats=out1, sc=sc1, node_attrs=inputs["node_attrs"])
        args, kwargs = capture_linear_call_args(linear_block, run)

    return linear_block, args, kwargs


def benchmark_linear_product(linear_block, call_args, call_kwargs, num_iterations=100, warmup=50, device="cuda"):
...
def run_linear_product_benchmark(device="cuda", batch_sizes=None, num_iterations=100, warmup=50):
    if batch_sizes is None:
        batch_sizes = [1, 4, 8, 16, 32]

    logging.info("=== MACE Linear Product Benchmark ===")
                ...
                try:
                    ...
                    for src_interaction in [0, 1]:
                        try:
                            ...
                            linear_block, call_args, call_kwargs = build_captured_args_for_linear_product(model, inputs, src_interaction)

                            results = benchmark_linear_product(
                                linear_block, call_args, call_kwargs, num_iterations, warmup, device
                            )
                            ...
                            logging.info(
                                f"      LinearProduct (from block {src_interaction}): "
                                f"Fwd: {results['forward_latency_ms']:.2f}ms, Bwd: {results['backward_latency_ms']:.2f}ms"
                            )
                        except Exception as e:
                            logging.info(f"      Error benchmarking linear product from block {src_interaction}: {e}")
                            continue
                except Exception as e:
...
def save_results(results):
    ...
    csv_path = os.path.join(results_dir, f"linear_product_benchmark_{timestamp}.csv")
...
def main():
    parser = argparse.ArgumentParser(description="MACE Linear Product Benchmark")
    ...
    results = run_linear_product_benchmark(
        device=args.device,
