"""
Comprehensive benchmark for linear_up step in MACE interactions[0] and interactions[1]

This script benchmarks the linear_up operation across:
...
def prepare_inputs(model, batch_dict, device, dtype, z_table):
    """Prepare inputs for linear_up testing"""
...
def benchmark_linear_up(block, inputs, num_iterations=100, warmup=50, device="cuda"):
    """Benchmark linear_up block with multiple iterations for stable timing"""
    
    def run_forward():
        # linear_up expects node_feats as input
        output = block(inputs["node_feats"])
        if device.startswith("cuda"):
            torch.cuda.synchronize()
        return output
    
    def run_backward(output):
        # Create a meaningful loss that ensures gradients flow to node_feats
...
def run_linear_up_benchmark(device="cuda", batch_sizes=None, num_iterations=100, warmup=50):
    """Run comprehensive linear_up benchmark"""
    
    if batch_sizes is None:
        batch_sizes = [1, 4, 8, 16, 32]
    
    logging.info(f"=== MACE linear_up Benchmark ===")
    ...
    for backend_name, backend_config in backend_configs:
        ...
        for precision_name, dtype in precision_configs:
            ...
            for i, interaction in enumerate(model.interactions):
                ...
                if hasattr(interaction, "linear_up"):
                    logging.info(f"        Has linear_up: {type(interaction.linear_up).__name__}")
                else:
                    logging.info(f"        No linear_up attribute")
            ...
            for batch_size in batch_sizes:
                ...
                try:
                    ...
                    for interaction_idx in [0, 1]:
                        # Extract just the linear_up component from the interaction block
                        interaction_block = model.interactions[interaction_idx]
                        if not hasattr(interaction_block, "linear_up"):
                            logging.info(f"      Block {interaction_idx}: No linear_up found")
                            continue
                        
                        linear_up_component = interaction_block.linear_up
                        logging.info(f"      Block {interaction_idx}: linear_up type: {type(linear_up_component).__name__}")
                        ...
                        # Prepare inputs specifically for linear_up component
                        # linear_up expects node_feats as input
                        linear_up_inputs = {
                            "node_feats": current_inputs["node_feats"]
                        }
                        
                        # Benchmark linear_up component (not the entire interaction block)
                        try:
                            results = benchmark_linear_up(
                                linear_up_component, linear_up_inputs, num_iterations, warmup, device
                            )
                            ...
                            logging.info(f"      Block {interaction_idx} linear_up: "
...
def save_results(results):
    ...
    csv_path = os.path.join(results_dir, f"linear_up_benchmark_{timestamp}.csv")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MACE linear_up Benchmark")
    ...
    # Run benchmark
    results = run_linear_up_benchmark(
        device=args.device,
