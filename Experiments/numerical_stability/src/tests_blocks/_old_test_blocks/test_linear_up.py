...
from src.mace_blocks import LinearLayer
...
def benchmark_linear_up(linear_up_fn, inputs, warmup=10, runs=50, precision_name=None, layer_number=None, device=None):
    ...
    if is_cuda_device(device):
        ...
        export_memory_snapshot(precision_name, layer_number)
...
def main():
    ...
    linear_up0 = model.interactions[0].linear_up
    linear_up1 = model.interactions[1].linear_up

    # turn their irreps into e3nn.Irreps to get dimensions
    in_ir0, out_ir0 = (
        Irreps(linear_up0.irreps_in),
        Irreps(linear_up0.irreps_out),
    )
    in_ir1, out_ir1 = (
        Irreps(linear_up1.irreps_in),
        Irreps(linear_up1.irreps_out),
    )

    logger.info(f"linear_up0.irreps_in: {linear_up0.irreps_in}")
    logger.info(f"linear_up0.irreps_out: {linear_up0.irreps_out}")
    logger.info(f"linear_up1.irreps_in: {linear_up1.irreps_in}")
    logger.info(f"linear_up1.irreps_out: {linear_up1.irreps_out}")

    logger.info(f"in_ir0: {in_ir0}")
    logger.info(f"out_ir0: {out_ir0}")
    logger.info(f"in_ir1: {in_ir1}")
    logger.info(f"out_ir1: {out_ir1}")

    N = 1000
    nf0 = torch.randn(N, in_ir0.dim, dtype=torch.float64).to(device)
    nf1 = torch.randn(N, in_ir1.dim, dtype=torch.float64).to(device)

    logger.info(f"nf0.shape: {nf0.shape}")
    logger.info(f"nf1.shape: {nf1.shape}")
