...
def run_conv_tp_benchmark(device="cuda", batch_sizes=None, num_iterations=100, warmup=50):
    ...
    # Create comparison CSV (e3nn vs cuEq side-by-side)
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    comparison_csv_path = create_comparison_csv(all_results, timestamp)
...
def create_comparison_csv(results, timestamp):
    """Create a comparison CSV showing e3nn vs cuEq side-by-side"""
    logging.info("\n--- Creating Comparison CSV ---")
    
    # Group results by backend, dtype, batch_size, and interaction
    ...
    for result in results:
        ...
        grouped[dtype][batch_size][interaction][backend] = result
    
    # Prepare comparison data
    comparison_data = []
    
    for dtype in ["fp64", "fp32", "bf16", "fp16"]:
        for batch_size in sorted(set().union(*[grouped[dtype].keys() for dtype in grouped.keys() if dtype in grouped])):
            for interaction in [0, 1]:
                if dtype in grouped and batch_size in grouped[dtype] and interaction in grouped[dtype][batch_size]:
                    config = grouped[dtype][batch_size][interaction]
                    
                    # Check if we have both e3nn and cuEq results
                    if "e3nn" in config and "cueq" in config:
                        e3nn_result = config["e3nn"]
                        cueq_result = config["cueq"]
                        ...
                        comparison_row = {
                            ...
                            "num_nodes": e3nn_result["num_nodes"],
                            "num_edges": e3nn_result["num_edges"],
                            
                            # e3nn results
                            "e3nn_forward_latency_ms": e3nn_result["forward_latency_ms"],
                            "e3nn_forward_std_ms": e3nn_result["forward_std_ms"],
                            "e3nn_backward_latency_ms": e3nn_result["backward_latency_ms"],
                            "e3nn_backward_std_ms": e3nn_result["backward_std_ms"],
                            "e3nn_memory_mb": e3nn_result["gpu_mem_peak_mb"],
                            "e3nn_forward_speedup_vs_fp64": e3nn_result.get("forward_speedup_vs_fp64", "N/A"),
                            "e3nn_backward_speedup_vs_fp64": e3nn_result.get("backward_speedup_vs_fp64", "N/A"),
                            
                            # cuEq results
                            "cueq_forward_latency_ms": cueq_result["forward_latency_ms"],
                            "cueq_forward_std_ms": cueq_result["forward_std_ms"],
                            "cueq_backward_latency_ms": cueq_result["backward_latency_ms"],
                            "cueq_backward_std_ms": cueq_result["backward_std_ms"],
                            "cueq_memory_mb": cueq_result["gpu_mem_peak_mb"],
                            "cueq_forward_speedup_vs_fp64": cueq_result.get("forward_speedup_vs_fp64", "N/A"),
                            "cueq_backward_speedup_vs_fp64": cueq_result.get("backward_speedup_vs_fp64", "N/A"),
                            
                            # Performance ratios (cuEq vs e3nn)
                            "forward_speedup_cueq_vs_e3nn": e3nn_result["forward_latency_ms"] / cueq_result["forward_latency_ms"] if cueq_result["forward_latency_ms"] > 0 else 0,
                            "backward_speedup_cueq_vs_e3nn": e3nn_result["backward_latency_ms"] / cueq_result["backward_latency_ms"] if cueq_result["backward_latency_ms"] > 0 else 0,
                            "memory_ratio_cueq_vs_e3nn": cueq_result["gpu_mem_peak_mb"] / e3nn_result["gpu_mem_peak_mb"] if e3nn_result["gpu_mem_peak_mb"] > 0 else 0,
                            
                            # Accuracy metrics (if available)
                            "e3nn_max_abs_error_fwd": e3nn_result.get("max_abs_error_fwd", "N/A"),
                            "e3nn_max_rel_error_fwd": e3nn_result.get("max_rel_error_fwd", "N/A"),
                            "e3nn_max_abs_error_bwd": e3nn_result.get("max_abs_error_bwd", "N/A"),
                            "e3nn_max_rel_error_bwd": e3nn_result.get("max_rel_error_bwd", "N/A"),
                            
                            "cueq_max_abs_error_fwd": cueq_result.get("max_abs_error_fwd", "N/A"),
                            "cueq_max_rel_error_fwd": cueq_result.get("max_rel_error_fwd", "N/A"),
                            "cueq_max_abs_error_bwd": cueq_result.get("max_abs_error_bwd", "N/A"),
                            "cueq_max_rel_error_bwd": cueq_result.get("max_rel_error_bwd", "N/A"),
                        }
                        
                        comparison_data.append(comparison_row)
                        
                        logging.info(f"  Added comparison: {dtype} batch={batch_size} interaction={interaction}")
                    elif "e3nn" in config:
                        # Only e3nn available (e.g., BF16/FP16 for cuEq)
                        e3nn_result = config["e3nn"]
                        comparison_row = {
                            "dtype": dtype,
                            "batch_size": batch_size,
                            ...
                            # cuEq results (N/A)
                            "cueq_forward_latency_ms": "N/A",
                            "cueq_forward_std_ms": "N/A",
                            "cueq_backward_latency_ms": "N/A",
                            "cueq_backward_std_ms": "N/A",
                            "cueq_memory_mb": "N/A",
                            "cueq_forward_speedup_vs_fp64": "N/A",
                            "cueq_backward_speedup_vs_fp64": "N/A",
                            
                            # Performance ratios (N/A)
                            "forward_speedup_cueq_vs_e3nn": "N/A",
                            "backward_speedup_cueq_vs_e3nn": "N/A",
                            "memory_ratio_cueq_vs_e3nn": "N/A",
                            ...
                            "cueq_max_abs_error_fwd": "N/A",
                            "cueq_max_rel_error_fwd": "N/A",
                            "cueq_max_abs_error_bwd": "N/A",
                            "cueq_max_rel_error_bwd": "N/A",
                        }
                        
                        comparison_data.append(comparison_row)
                        logging.info(f"  Added e3nn-only comparison: {dtype} batch={batch_size} interaction={interaction}")
    
    # Save comparison CSV
    if comparison_data:
        comparison_csv_path = f"Experiments/numerical_stability/src/tests_blocks/results/conv_tp_comparison_{timestamp}.csv"
        ...
        with open(comparison_csv_path, 'w', newline='') as csvfile:
            if comparison_data:
                ...
                logging.info(f"  Comparison CSV saved: {len(comparison_data)} rows")
                logging.info(f"  Comparison CSV path: {comparison_csv_path}")
            else:
                logging.warning("  No comparison data to write!")
        
        return comparison_csv_path
    else:
        logging.warning("  No comparison data generated!")
        return None
