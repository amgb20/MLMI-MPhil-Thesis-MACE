"""
Benchmark for Symmetric Contraction (product.symmetric_contractions) in MACE

This benchmarks the symmetric contraction sub-block across:
- Backends: e3nn (reference) and cuEq (if available)
- Precisions: fp64, fp32, bf16/fp16 where supported
- Batch sizes
- Forward and backward latency

Inputs to symmetric_contractions are constructed exactly as in models.product:
- For interaction block 0: we run interactions[0](...) and feed its output node features
- For interaction block 1: we additionally run the product/update path if present, then interactions[1](...), and feed its output
...
def capture_symmetric_call_args(symm_block, run_callable):
    """Monkeypatch symm_block.forward to capture exact args passed during a normal forward."""
    captured = {}
    orig_forward = symm_block.forward
    def wrapper(*args, **kwargs):
        captured['args'] = args
        captured['kwargs'] = kwargs
        return orig_forward(*args, **kwargs)
    symm_block.forward = wrapper
    try:
        run_callable()
    finally:
        symm_block.forward = orig_forward
    return captured.get('args', ()), captured.get('kwargs', {})


def build_captured_args_for_symmetric_contraction(model, inputs, interaction_idx):
    # Prepare a run that executes the product[idx] as in the model to capture exact symmetric_contractions inputs
    prod_idx = interaction_idx
    symm_block = model.products[prod_idx].symmetric_contractions
    ...
    if interaction_idx == 0:
        def run():
            ...
            _ = model.products[0](node_feats=out0, sc=sc0, node_attrs=inputs["node_attrs"])  # triggers symmetric_contractions
        args, kwargs = capture_symmetric_call_args(symm_block, run)
    else:
        def run():
            out0, sc0 = model.interactions[0](
                node_attrs=inputs["node_attrs"],
            ...
            h1 = model.products[0](node_feats=out0, sc=sc0, node_attrs=inputs["node_attrs"]) 
            try:
                if hasattr(model, "updates") and len(model.updates) > 0:
                    h1 = model.updates[0](node_feats=h1, node_attrs=inputs["node_attrs"]) 
            except Exception:
                pass
            ...
            _ = model.products[1](node_feats=out1, sc=sc1, node_attrs=inputs["node_attrs"])  # triggers symmetric_contractions
        args, kwargs = capture_symmetric_call_args(symm_block, run)

    return symm_block, args, kwargs


def benchmark_symmetric_contraction(symm_block, call_args, call_kwargs, num_iterations=100, warmup=50, device="cuda"):
    # Identify tensor argument to differentiate for gradients (first tensor arg)
    args = list(call_args)
    tensor_arg_idx = None
    for i, a in enumerate(args):
        if isinstance(a, torch.Tensor):
            tensor_arg_idx = i
            break
    if tensor_arg_idx is None:
        raise RuntimeError("No tensor argument found for symmetric_contractions call")
    bench_tensor = args[tensor_arg_idx].clone().detach().requires_grad_()
    bench_tensor.retain_grad()
    args[tensor_arg_idx] = bench_tensor

    def run_forward():
        out = symm_block(*args, **call_kwargs)
...
def run_symmetric_contraction_benchmark(device="cuda", batch_sizes=None, num_iterations=100, warmup=50):
    if batch_sizes is None:
        batch_sizes = [1, 4, 8, 16, 32]

    logging.info("=== MACE symmetric_contraction Benchmark ===")
        ...
        for precision_name, dtype in precision_configs:
            ...
            for batch_size in batch_sizes:
                ...
                try:
                    ...
                    # Bench both interaction outputs as inputs
                    for src_interaction in [0, 1]:
                        try:
                            # Capture exact call args as executed by model for the proper product index
                            symm_block, call_args, call_kwargs = build_captured_args_for_symmetric_contraction(model, inputs, src_interaction)

                            results = benchmark_symmetric_contraction(
                                symm_block, call_args, call_kwargs, num_iterations, warmup, device
                            )
                            ...
                            logging.info(
                                f"      SymmetricContraction (from block {src_interaction}): "
                                f"Fwd: {results['forward_latency_ms']:.2f}ms, Bwd: {results['backward_latency_ms']:.2f}ms"
                            )
                        except Exception as e:
                            logging.info(f"      Error benchmarking symmetric_contraction from block {src_interaction}: {e}")
                            continue
                except Exception as e:
                    logging.info(f"      Error with batch size {batch_size}: {e}")
                    continue

    # Compute speedups vs FP64 per backend, batch size, and src_interaction
    fp64_refs = {}
    for r in all_results:
        if r["dtype"] == "fp64":
            key = (r["backend"], r["batch_size"], r["src_interaction"])
            fp64_refs[key] = r
    for r in all_results:
        if r["dtype"] == "fp64":
            r.update({
...
def save_results(results):
    ...
    column_mapping = {
        ...
        "src_interaction": "Source Interaction",
    ...
    csv_path = os.path.join(results_dir, f"symmetric_contraction_benchmark_{timestamp}.csv")
...
def generate_summary_report(results):
    ...
    for dtype in ["fp64", "fp32", "bf16", "fp16"]:
        ...
        for src in [0, 1]:
            e3_src = [x for x in e3 if x["src_interaction"] == src]
            cq_src = [x for x in cq if x["src_interaction"] == src]
            if not e3_src or not cq_src:
                logging.info(f"  No comparable results for src_interaction={src}"); continue
            e3r = e3_src[0]; cqr = cq_src[0]
            ...
            logging.info(f"  src={src}: Forward e3nn {e3r['forward_latency_ms']:.2f}ms ± {e3r.get('forward_std_ms',0):.2f}, cuEq {cqr['forward_latency_ms']:.2f}ms ± {cqr.get('forward_std_ms',0):.2f}, speedup {fwd_speedup:.2f}x")
            logging.info(f"  src={src}: Backward e3nn {e3r['backward_latency_ms']:.2f}ms ± {e3r.get('backward_std_ms',0):.2f}, cuEq {cqr['backward_latency_ms']:.2f}ms ± {cqr.get('backward_std_ms',0):.2f}, speedup {bwd_speedup:.2f}x")


def main():
    parser = argparse.ArgumentParser(description="MACE symmetric_contraction Benchmark")
    ...
    results = run_symmetric_contraction_benchmark(
        device=args.device,
