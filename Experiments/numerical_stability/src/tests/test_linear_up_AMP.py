...
class LinearLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        output = self.impl(x)
        # Preserve input dtype only when autocast is NOT active
        if (not torch.is_autocast_enabled()) and (output.dtype != x.dtype):
...
def run_linear_backward_pass(
    ...
    *,
    ...
    if device is None:
        try:
            device = next(layer.impl.parameters()).device
        except StopIteration:
            device = x.device

    if amp and device.type == "cuda" and amp_dtype == torch.float16 and use_grad_scaler:
        ...
        # Manually unscale grads since we don't use an optimizer here
        if x.grad is not None:
            x.grad.data.div_(scale_value)
        for p in layer.impl.parameters():
            if p.grad is not None:
                p.grad.data.div_(scale_value)
    else:
        # BF16 AMP or non-AMP
        cm = (
            ...
            if (amp and device.type == "cuda")
                ...
                torch.autocast("cpu", dtype=amp_dtype)
                if (amp and device.type == "cpu" and amp_dtype == torch.bfloat16)
                else nullcontext()
            )
        )
        with cm:
...
def test_precision_accuracy(layer, input_tensor, reference_output, precision_name):
    ...
    with torch.inference_mode():
        ...
        # Handle AMP modes
        try:
            amp_mode = precision_name.upper().startswith("AMP")
        except Exception:
            amp_mode = False

        if amp_mode:
            amp_dtype = torch.float16 if "FP16" in precision_name.upper() else torch.bfloat16
            try:
                device = next(layer.impl.parameters()).device
            except StopIteration:
            ...
            cm = (
                ...
                if device.type == "cuda"
                    ...
                    torch.autocast("cpu", dtype=amp_dtype)
                    if amp_dtype == torch.bfloat16 and device.type == "cpu"
                    else nullcontext()
                )
            )
            with cm:
...
def benchmark_precision_effects(irreps_in, irreps_out, input_tensor, device, warmup=10, runs=50, use_cueq=False, layer_number=None):
    ...
    # Create layers at different precisions - FP64, FP32, and AMP variants when supported
    precisions: list[tuple[torch.dtype | None, str]] = [
        (torch.float64, "FP64"),
        (torch.float32, "FP32"),
    ]
    # Add AMP modes conditionally
    if is_cuda_device(device):
        # Prefer both AMP FP16 and AMP BF16 when CUDA is available
        precisions.append((None, "AMP_FP16"))
        if torch.cuda.is_bf16_supported():
            precisions.append((None, "AMP_BF16"))
    else:
        # CPU AMP: only BF16 is generally supported
        try:
            # If torch supports cpu autocast bf16
            _ = torch.autocast
            precisions.append((None, "AMP_BF16"))
        except Exception:
            pass
    ...
    for math_dtype, precision_name in precisions:
        ...
        if precision_name == "FP64":
        ...
        elif precision_name.startswith("AMP"):
            # AMP keeps params in FP32 and uses autocast for compute; inputs cast to AMP compute dtype for more effect
            layer = LinearLayer(
                irreps_in,
                irreps_out,
                use_cueq=use_cueq,
            ...
            copy_state_dict_cast(fp64_layer, layer, dtype=torch.float32, device=device)
            if "FP16" in precision_name:
                # For CUDA FP16, feed FP16 inputs
                input_precision = input_tensor.to(torch.float16 if is_cuda_device(device) else torch.float32)
            else:
                # BF16 path
                input_precision = input_tensor.to(torch.bfloat16)
        else:
            raise ValueError(f"Unknown precision name: {precision_name}")
        ...
        if precision_name.startswith("AMP"):
            amp_dtype = torch.float16 if "FP16" in precision_name else torch.bfloat16
            loss, input_grad, param_grads = run_linear_backward_pass(
                layer,
                input_precision,
                amp=True,
                amp_dtype=amp_dtype,
                device=device,
                use_grad_scaler=(amp_dtype == torch.float16 and is_cuda_device(device)),
            )
        else:
...
def analyze_precision_results(results):
    ...
    """
    logger.info(f"\n{'='*60}")
    logger.info(f"PRECISION EFFECTS ANALYSIS (FP32/AMP vs FP64)")
    logger.info(f"{'='*60}")
    ...
    if 'AMP_FP16' in results:
        amp16_time = results['AMP_FP16']['time_ms']
        logger.info(f"  AMP FP16: {amp16_time:.2f} ms ({fp64_time/amp16_time:.2f}x speedup vs FP64)")
    if 'AMP_BF16' in results:
        ampb_time = results['AMP_BF16']['time_ms']
        logger.info(f"  AMP BF16: {ampb_time:.2f} ms ({fp64_time/ampb_time:.2f}x speedup vs FP64)")
    ...
    if 'AMP_FP16' in results:
        amp16_mem = results['AMP_FP16']['peak_mem_mb']
        logger.info(f"  AMP FP16: {amp16_mem:.2f} MB ({fp64_mem/amp16_mem:.2f}x memory reduction vs FP64)")
    if 'AMP_BF16' in results:
        ampb_mem = results['AMP_BF16']['peak_mem_mb']
        logger.info(f"  AMP BF16: {ampb_mem:.2f} MB ({fp64_mem/ampb_mem:.2f}x memory reduction vs FP64)")
    ...
    if 'AMP_FP16' in results:
        a16 = results['AMP_FP16']['accuracy']
        logger.info(f"  AMP FP16 vs FP64:")
        logger.info(f"    Max Absolute Error: {a16['max_abs_error']:.2e}")
        logger.info(f"    Mean Absolute Error: {a16['mean_abs_error']:.2e}")
        logger.info(f"    Max Relative Error: {a16['max_rel_error']:.2e}")
        logger.info(f"    Mean Relative Error: {a16['mean_rel_error']:.2e}")
    if 'AMP_BF16' in results:
        ab = results['AMP_BF16']['accuracy']
        logger.info(f"  AMP BF16 vs FP64:")
        logger.info(f"    Max Absolute Error: {ab['max_abs_error']:.2e}")
        logger.info(f"    Mean Absolute Error: {ab['mean_abs_error']:.2e}")
        logger.info(f"    Max Relative Error: {ab['max_rel_error']:.2e}")
        logger.info(f"    Mean Relative Error: {ab['mean_rel_error']:.2e}")
    ...
    if 'AMP_FP16' in results:
        logger.info(f"  ✓ AMP FP16 speedup: {fp64_time/results['AMP_FP16']['time_ms']:.2f}x vs FP64")
        logger.info(f"  ✓ AMP FP16 memory reduction: {fp64_mem/results['AMP_FP16']['peak_mem_mb']:.2f}x vs FP64")
    if 'AMP_BF16' in results:
        logger.info(f"  ✓ AMP BF16 speedup: {fp64_time/results['AMP_BF16']['time_ms']:.2f}x vs FP64")
        logger.info(f"  ✓ AMP BF16 memory reduction: {fp64_mem/results['AMP_BF16']['peak_mem_mb']:.2f}x vs FP64")
...
def benchmark_linear_up(linear_up_fn, inputs, warmup=10, runs=50, precision_name=None, layer_number=None, device=None):
    ...
    # For AMP modes, avoid tracing to keep autocast semantics
    use_amp = isinstance(precision_name, str) and precision_name.upper().startswith("AMP")
    if not use_amp:
    ...
    with torch.inference_mode():
        if not use_amp:
            ...
            # Warm first call for AMP path too
            amp_dtype = torch.float16 if "FP16" in precision_name.upper() else torch.bfloat16
            cm = (
                torch.autocast("cuda", dtype=amp_dtype)
                if (is_cuda_device(device))
        ...
        if is_cuda_device(device):
            ...
            for _ in range(runs):
                ...
                if not use_amp:
                    ...
                    amp_dtype = torch.bfloat16  # CPU AMP assumed BF16
                    cm = torch.autocast("cpu", dtype=amp_dtype)
                    with cm:
                        if isinstance(inputs, tuple):
                            linear_up_fn(*inputs)
                        else:
    ...
    if not use_amp:
        ...
        with cm:
            test_output = linear_up_fn(*inputs) if isinstance(inputs, tuple) else linear_up_fn(inputs)
        # Report the actual compute dtype under autocast
        output_dtype = test_output.dtype
