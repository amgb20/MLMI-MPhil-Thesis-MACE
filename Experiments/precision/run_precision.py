###########################################################################################
# Precision analysis script for MACE
# Authors: Ilyes Batatia, Gregor Simm, David Kovacs
# Authors 2: Alexandre Benoit
...
from configure_model_precision import configure_model
...
from mace.tools.multihead_tools import (HeadConfig, assemble_mp_data,
                                        dict_head_to_dataclass,
                                        prepare_default_head, prepare_pt_head)
from mace.tools.run_train_utils import (combine_datasets,
                                        load_dataset_for_path,
                                        normalize_file_paths)
from mace.tools.scripts_utils import (LRScheduler, SubsetCollection,
                                      check_path_ase_read,
                                      convert_to_json_format, dict_to_array,
                                      extract_config_mace_model,
                                      get_atomic_energies,
                                      get_avg_num_neighbors,
                                      get_config_type_weights,
                                      get_dataset_from_xyz,
                                      get_files_with_suffix, get_loss_fn,
                                      get_optimizer, get_params_options,
                                      get_swa, print_git_commit,
                                      remove_pt_head, setup_wandb)
...
def run(args) -> None:
    ...
    # Print block sizes for each interaction block if flag is set
    if args.show_block_sizes:
        logging.info("\n===========BLOCK SIZES===========")
        for i, interaction in enumerate(model.interactions):
            logging.info(f"\nInteraction Block {i+1}:")
            interaction.print_block_sizes()
        logging.info("")
