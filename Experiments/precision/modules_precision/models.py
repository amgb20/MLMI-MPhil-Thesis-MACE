...
from .utils import (compute_fixed_charge_dipole, get_atomic_virials_stresses,
                    get_edge_vectors_and_lengths, get_outputs,
                    get_symmetric_displacement, prepare_graph)
...
class ScaleShiftMACE(MACE):
    ...
    def forward(
        ...
        logging.info(
            f"==== Converting and saving Embedding outputs to fp64 (default) ===="
        )
        ...
        for i, (interaction, product, readout) in tqdm(
            enumerate(zip(self.interactions, self.products, self.readouts))
        ):
            node_attrs_slice = data["node_attrs"]
            if is_lammps and i > 0:
                node_attrs_slice = node_attrs_slice[: lammps_natoms[0]]
            ...
            rel_diff_fp32 = torch.abs(tensor_fp64 - tensor_fp32) / (
                torch.abs(tensor_fp64) + 1e-12
            )
            ...
            logging.info(
                f"fp64 vs fp32 - Absolute difference - Max: {max_abs_diff_fp32:.2e}, Mean: {mean_abs_diff_fp32:.2e}, Std: {std_abs_diff_fp32:.2e}"
            )
            logging.info(
                f"fp64 vs fp32 - Relative difference - Max: {max_rel_diff_fp32:.2e}, Mean: {mean_rel_diff_fp32:.2e}, Std: {std_rel_diff_fp32:.2e}"
            )
            ...
            rel_diff_fp16 = torch.abs(tensor_fp64 - tensor_fp16) / (
                torch.abs(tensor_fp64) + 1e-12
            )
            ...
            logging.info(
                f"fp64 vs fp16 - Absolute difference - Max: {max_abs_diff_fp16:.2e}, Mean: {mean_abs_diff_fp16:.2e}, Std: {std_abs_diff_fp16:.2e}"
            )
            logging.info(
                f"fp64 vs fp16 - Relative difference - Max: {max_rel_diff_fp16:.2e}, Mean: {mean_rel_diff_fp16:.2e}, Std: {std_rel_diff_fp16:.2e}"
            )
        ...
        for precision in ["fp32", "fp16"]:
            ...
            std_abs_mean = np.std(
                abs_means
            )  # Standard deviation of means across layers
            ...
            logging.info(
                f"==== {precision.upper()} AVERAGE ACROSS {len(abs_means)} LAYERS ===="
            )
            logging.info(
                f"Absolute - Avg Mean: {avg_abs_mean:.2e} ± {std_abs_mean:.2e}, Avg Max: {avg_abs_max:.2e}, Avg Std: {avg_abs_std:.2e}"
            )
            logging.info(
                f"Relative - Avg Mean: {avg_rel_mean:.2e} ± {std_rel_mean:.2e}, Avg Max: {avg_rel_max:.2e}, Avg Std: {avg_rel_std:.2e}"
            )
