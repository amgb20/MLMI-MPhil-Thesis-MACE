...
class InteractionBlock(torch.nn.Module):
    ...
    def describe_irreps(self, tag, ir):
        # works old & new e3nn
        def dim(ir):
            d = getattr(ir, "dim")
...
class RealAgnosticInteractionBlock(InteractionBlock):
    def _setup(self) -> None:
        ...
        # # # Set conv_fusion only when the TensorProduct backend supports it.
        # # # When cuEq acceleration is used, ``conv_tp`` is a
        # # # ``ChannelWiseTensorProduct`` which does not accept the additional
        # # # ``edge_index`` argument used in the fused convolution path.  In that
        # # # case ``conv_fusion`` should not be enabled.
        # if self.oeq_config and self.oeq_config.conv_fusion and not (self.cueq_config and self.cueq_config.enabled):
        #     self.conv_fusion = self.oeq_config.conv_fusion
